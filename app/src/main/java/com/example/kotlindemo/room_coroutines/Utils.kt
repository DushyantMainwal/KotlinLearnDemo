...
fun formatNights(nights: List<SleepNight>, resources: Resources): Spanned {
    ...
    sb.apply {
        ...
        nights.forEach {
            ...
            if (it.stopTime != it.startTime) {
                append(resources.getString(R.string.end_time))
                append("\t${convertLongToDateString(it.stopTime)}<br>")
                append(resources.getString(R.string.quality))
                append("\t${convertNumericQualityToString(it.qualityIndex, resources)}<br>")
                append(resources.getString(R.string.hours_slept))
                // Hours
                append("\t ${it.stopTime.minus(it.startTime) / 1000 / 60 / 60}:")
                // Minutes
                append("${it.stopTime.minus(it.startTime) / 1000 / 60}:")
                // Seconds
                append("${it.stopTime.minus(it.startTime) / 1000}<br><br>")
            }
        }
    }
